        ...
        for (int j = i+2; j < patients.size() ; ++j) {
         ...
         if (((totalDistance+findDistanceAB(patients[i], patients[i+1])+ findDistanceAB(patients[i+1],warehouse)) < getMaxDistance())
             && ((totalWeight + findWeightAB(path.pB)) < 3000)){
                 ...
                 totalDistance = totalDistance + path.distanceAtoB;
                 totalWeight = totalWeight + path.weightAtoB;
...
int Route::findAngleAB(Patient pA, Patient pB)
{
     ...
     a = round(atan2((pB.getLocation().cY - pA.getLocation().cY),(pB.getLocation().cX - pA.getLocation().cX))*1000.0/3.1416);

     if (a > 0){
        return int(a);
     }else if (a < 0){   // If the angle is negative, adding 2000 will result in an equivalent positive angle
        return int(a)+2000;
     }
}
...
int Route::findWeightAB(Patient patient)
{
     ...
     for(int i = 0; i < orders.size(); i++){
        if(patient.getID() == orders[i].getPatientID()){
            totalW = totalW + orders[i].getOrderWeight();
        }
     }
     return totalW;
